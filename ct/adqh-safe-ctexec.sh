...
install_app_in_ct() {
  ...
  msg_info "Installing SAFE systemd units in CT ${ctid}"
  exec_ct "$ctid" "install -D -m 0644 /opt/adqh/deploy/adqh-safe.service /etc/systemd/system/adqh.service"
  exec_ct "$ctid" "install -D -m 0644 /opt/adqh/deploy/adqh-safe.timer /etc/systemd/system/adqh.timer"
  exec_ct "$ctid" "systemctl daemon-reload && systemctl enable --now adqh.timer"
  msg_ok "Installed SAFE systemd units in CT ${ctid}"
}
...
echo -e "${CREATING}${GN}${APP} SAFE setup has been successfully initialized in CT ${CTID_NOW}!${CL}"
echo -e "${INFO}${YW} Config: /etc/adqh/adqh.env${CL}"
echo -e "${INFO}${YW} Data: /var/lib/adqh${CL}"
echo -e "${INFO}${YW} Run once in CT: pct exec ${CTID_NOW} -- bash -lc '/opt/adqh/.venv/bin/python -m adqh.cli_safe run-once --dry-run'${CL}"
